...
final class ApiDataViewController: UIViewController, UISearchResultsUpdating, UISearchBarDelegate {
   ...
   private var RelamPostElement: Results<PostElement>? = nil
   ...
   private func fetchData() {
        service.downloadService { [weak self] result in
            ...
            self.reloaded()
            DispatchQueue.main.async {
                self.RealmSaveData(model: result)
            }
        }
    }
}
...
extension ApiDataViewController: UITableViewDelegate, UITableViewDataSource {
    internal func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if resultSearchController.isActive {
            ...
            return RelamPostElement?.count ?? Post.count
            }
...
extension ApiDataViewController {
   private func RealmSaveData(model:[PostElement]) -> (){
        
            do{
                _ = try Realm().objects(PostElement.self).sorted(byKeyPath: "_id", ascending: true)
                let realm = try? Realm()
                    if realm!.isEmpty == false {
                        ...
                        try realm!.write {
                            realm?.add(model)
                            RelamPostElement = realm?.objects(PostElement.self)
                        }
                    }
            } catch {
            print(error)
